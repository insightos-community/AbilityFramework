...
private:
    ...
    static void* _Allocate(_Alloc _Al, const size_t _Size) {
        if constexpr (default_initializable<_Alloc>
...
private:
    ...
    static void* _Allocate(const _ProtoAlloc& _Proto, size_t _Size) {
        ...
        if constexpr (default_initializable<_Alloc>
            ...
            const size_t _Count
                = (_Size + sizeof(_Dealloc_fn) + sizeof(_Al) + _Align - 1) / sizeof(_Aligned_block);
            void* const _Ptr = _Al.allocate(_Count);
            ::memcpy(static_cast<char*>(_Ptr) + _Size, &_Dealloc, sizeof(_Dealloc));
            _Size += sizeof(_Dealloc_fn);
            const auto _Al_address
...
public:
    static void* operator new(const size_t _Size) { // default: new/delete
        ...
        const _Dealloc_fn _Dealloc
            = [](void* const _Ptr, const size_t _Size) { ::operator delete[](_Ptr); };
...
template <::std::ranges::range _Rng, class _Alloc = allocator<byte>>
struct elements_of {
...
public:
    ...
    /* [[nodiscard]] */ suspend_always initial_suspend() noexcept { return {}; }
    ...
    // clang-format off
        ...
        [[nodiscard]] auto yield_value(
            elements_of<generator<_Rty, _Vty, _Alloc>&&, _Unused> _Elem) noexcept {
        // clang-format on
    ...
    // clang-format off
        ...
        [[nodiscard]] auto yield_value(elements_of<_Rng, _Alloc> _Elem) noexcept {
        // clang-format on
    ...
    void unhandled_exception() {
        if (_Info) { _Info->_Except = ::std::current_exception(); }
        else { throw; }
    }
...
private:
    ...
    struct _Final_awaiter {
        ...
        template <class _Promise>
        [[nodiscard]] coroutine_handle<> await_suspend(coroutine_handle<_Promise> _Handle
        ) noexcept {
#ifdef __cpp_lib_is_pointer_interconvertible
            static_assert(is_pointer_interconvertible_base_of_v<_Gen_promise_base, _Promise>);
#endif // __cpp_lib_is_pointer_interconvertible
    ...
    struct _Nested_awaitable {
        ...
        [[nodiscard]] coroutine_handle<_Gen_promise_base> await_suspend(
            coroutine_handle<_Promise> _Current
        ) noexcept {
#ifdef __cpp_lib_is_pointer_interconvertible
            static_assert(is_pointer_interconvertible_base_of_v<_Gen_promise_base, _Promise>);
#endif // __cpp_lib_is_pointer_interconvertible
            ...
            if (_Parent_promise._Info) { _Nested._Root = _Parent_promise._Info->_Root; }
            else { _Nested._Root = _Nested._Parent; }
        ...
        void await_resume() {
            if (_Nested._Except) { ::std::rethrow_exception(::std::move(_Nested._Except)); }
        }
    };
...
public:
    ...
    [[nodiscard]] bool operator==(default_sentinel_t) const noexcept { return _Coro.done(); }
...
private:
    ...
    explicit _Gen_iter(
        _Gen_secret_tag, coroutine_handle<_Gen_promise_base<_Gen_yield_t<_Ref>>> _Coro_
    ) noexcept
        : _Coro{_Coro_} {}
...
public:
    ...
    [[nodiscard]] default_sentinel_t end() const noexcept { return default_sentinel; }
...
using draft::elements_of;
using draft::generator;
