...
void TaskManager::update() {
    ...
    for (auto it = active_handles.begin(); it != active_handles.end();) {
        ...
        case TaskState::fine: {
            handle->send(); // give it one round to execute
            ++it;
            continue;
        } break;
        ...
        case TaskState::cancelled: {
            handle->close();
            it = active_handles.erase(it); // clear the handle
        };
        default: {
            // the condition this task depends on is not ready yet
        } break;
        }
    }
}
...
void build_api(std::shared_ptr<TaskManager> task_entry, httplib::Server& server) {
    ...
    // define GET /api/task endpoint, returns UUID list of all tasks
    server.Get("/api/task", [task_entry](const Request& req, Response& res) {
        std::lock_guard _lk(task_entry->m);
        nlohmann::json payload; // create a JSON object to store the task UUID list
        if (req.params.contains("active")) { // if the request parameter contains the active field
    ...
    // define GET /api/task/:id endpoint, returns status of the task with the given UUID
    server.Get("/api/task/:id", [task_entry](const Request& req, Response& res) {
        std::lock_guard _lk(task_entry->m);
        auto uuid = uuids::uuid::from_string(req.path_params.at("id")); // get UUID from request path
        if (!uuid) { throw std::invalid_argument("input is not valid uuid"); }
        auto it = task_entry->tasks.find(*uuid); // use std::map's find method to find the task
        if (it == task_entry->tasks.end()) {
            throw make_error<std::invalid_argument>("no such task of uuid ", to_string(*uuid));
        }
        // convert task status to JSON format and respond
    ...
    server.Get("/api/message", [](const Request& req, Response& res) {
        ...
        // format JSON object to string and output to console
        VLOG(1) << "try to send to task mgrdata: " << data.dump(2);
        send_msg("another_module", "TaskMgr", "add_task/json", data);
        res.set_content("ok, GET /api/message", "text/plain");
    });

    // define GET /api/extra endpoint, returns "ok"
    ...
    // define POST /api/task endpoint, for creating tasks
...
expected<void, std::string> TaskManager::on_receive(const message_bus::Message& message) {
    ...
    if (message.router.operation == "add_task/json") {
        ...
        // only pass needed function parameters; pass parameters to the preset function when creating the task
    ...
    // parse json to get task type, but do not execute immediately make_task
    else if (message.router.operation == "make_task/json") {
        VLOG(1) << "receive: 'make_task/json'";
        auto j = nlohmann::json::parse(message.content);
        VLOG(1) << "receive data:" << j.dump(2);
        std::string name = j.at("name").get<std::string>();
        std::string type = j.at("type").get<std::string>(); // task type

        // convert json to TaskPtr

        return {};
    }

    // get a TaskPtr from add_task/raw extra field, then execute
        ...
        // dynamically cast to message_bus::Extra<TaskPtr>
...
// test sending message with extra
void send_extra_msg(
