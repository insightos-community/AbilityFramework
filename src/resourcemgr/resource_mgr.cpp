...
// send POST request; on success returns res->body, j["result"] == "succcess" means the target machine executed successfully
...
CREATE TABLE IF NOT EXISTS AbilityCRBasic(
  instance_id TEXT PRIMARY KEY, -- instance id (as primary key)
  ability_name TEXT NOT NULL, -- ability class name, corresponding spec/abilityName
  ability_version TEXT NOT NULL, -- ability version
  instance_name TEXT NOT NULL, -- instance name, corresponding metadata/name
  insert_time INTEGER NOT NULL, -- creation time, last updated time, unix seconds format
  update_time INTEGER NOT NULL, -- last updated time, unix seconds format
  autostart INTEGER, -- 0 or missing indicates false, 1 indicates true (auto-spawn only once at framework start)
  auto_started INTEGER DEFAULT 0, -- whether already executed autoStart (0=not executed, 1=executed)
  keep_alive INTEGER DEFAULT 0, -- keep-alive (0=no keep-alive, 1=auto-restart after exit)
  singleton INTEGER DEFAULT 1, -- singleton (1=singleton/default, 0=allows multiple instances)
  cr_detail TEXT NOT NULL, -- store CR as-is in json format
  cr_filepath TEXT -- iforiginates fromafile, then record the file path, otherwise empty
);
)sql";

constexpr char SQL_CREATE_ABILITY_CR_FILE_INFO[] = R"sql(
CREATE TABLE IF NOT EXISTS AbilityCRFileInfo(
  instance_id TEXT PRIMARY KEY, -- cr instance id
  cr_file_path TEXT NOT NULL, -- ifCRoriginates fromfile, then record the file path
  cr_file_digest TEXT NOT NULL, -- crfile content sha256
  cr_file_detail BLOB NOT NULL, -- specific CR file
  cr_file_extenstion TEXT NOT NULL -- crfile extension, currently only supports "yaml"
);
)sql";
...
CREATE TABLE IF NOT EXISTS DeviceCRBasic(
  instance_id TEXT PRIMARY KEY, -- instance id (as primary key)
  device_name TEXT NOT NULL, -- ability class name, corresponding spec/abilityName
  device_version TEXT NOT NULL, -- ability version
  instance_name TEXT NOT NULL, -- instance name, corresponding metadata/name
  insert_time INTEGER NOT NULL, -- creation time, last updated time, unix seconds format
  update_time INTEGER NOT NULL, -- last updated time, unix seconds format
  cr_detail TEXT NOT NULL, -- store CR as-is in json format
...
CREATE TABLE IF NOT EXISTS AbilityCRDBasic(
  ability_name TEXT NOT NULL, -- ability class name, corresponding spec/abilityName
  ability_version TEXT NOT NULL, -- ability version
  crd_detail TEXT NOT NULL, -- store CRD as-is in json format
...
CREATE TABLE IF NOT EXISTS CROwnership(
  parent_id TEXT NOT NULL, -- dependent id
  parent_kind TEXT NOT NULL, -- "ability" or "device"
  parent_address TEXT NOT NULL, -- dependent ip address
  child_id TEXT NOT NULL, -- dependent id
  child_kind TEXT NOT NULL, -- dependent type, "ability" or "device"
  is_shared INTEGER NOT NULL, -- 0 indicates shared, 1 indicates exclusive
  PRIMARY KEY (parent_id, parent_kind, child_id, child_kind)
);
)sql";

// AbilityInstance: runtime row, one per running/starting instance.
// and AbilityCRBasic(template)is N:1 relation, via cr_id associated.
// delete this row immediately after the instance exits(regardless singleton/replica).
// spec_snapshot records the CR JSON at startup, ensuring that even if the template is modified afterwards,
// the running ability still works with the original spec.
constexpr char SQL_CREATE_ABILITY_INSTANCE[] = R"sql(
CREATE TABLE IF NOT EXISTS AbilityInstance(
  instance_id TEXT PRIMARY KEY, -- runtime instance UUID(and cr_id different)
  cr_id TEXT, -- belongs totemplate (AbilityCRBasic.instance_id)
  cr_name TEXT NOT NULL, -- corresponding CR metadata.name
  instance_name TEXT NOT NULL, -- instance display name(cr_name-<id before 8 bit>)
  ability_name TEXT NOT NULL, -- ability class name
  ability_version TEXT NOT NULL, -- ability version
  pid INTEGER DEFAULT 0, -- process PID(not used yet)
  state TEXT DEFAULT 'Inactive', -- lifecycle state
  start_time INTEGER, -- start time (unix sec)
  stop_time INTEGER, -- stop time
  spec_snapshot TEXT, -- at startup AbilityCR JSON snapshot
  detail TEXT -- extra info JSON(IPCPort/abilityPort, etc.)
);
)sql";
...
void db_create_cr_crd_tables() {
    ...
    // AbilityInstance is a pure runtime transient table; the schema was upgraded again (added cr_id/spec_snapshot column). 
    // just rebuild directly, no need to migrate old data.
...
std::optional<AbilityPackage> read_package(const Path& pkg_dir) {
    ...
    try {
        ...
        // synthesize AbilityCRD from manifest for downstream modules
        AbilityCRD crd;
...
// note: in g++ 10, directory_iterator cannot be used directly inside range-based for loops;
// for compatibility, proceed step by step
...
// insert_time INTEGER NOT NULL, -- creation time, last updated time, unix seconds format
// update_time INTEGER NOT NULL, -- last updated time, unix seconds format
// autostart INTEGER, -- 0 or missing indicates false, 1 indicates true
// cr_detail TEXT NOT NULL, -- store CR as-is in json format
// cr_filepath TEXT -- iforiginates fromafile, then record the file path, otherwise empty
// );
...
// add sub-ability cr and sub-ability items for composite and abstract abilities
...
// add device items for the ability
...
// add device entry for the ability
...
bool ResourceManager::judge_ability_exist(const uuids::uuid& abilityInstance_id) const {
    ...
    // check CR template table (AbilityCRBasic) + runtime instancetable (AbilityInstance)
    // occupation can pass template id or instance id; both must hit
    constexpr char SQL[] = R"sql(
        SELECT EXISTS(
...
std::vector<std::string> ResourceManager::get_heartbeats_by_ability_name(
    ...
    // query running instances of this ability class name in the AbilityInstance table
...
uuids::uuid ResourceManager::create_ability_instance(const AbilityCR& cr_template) {
    ...
    // instance display name:<template name>-<id before 8 bit>
    std::string display_name = cr_template.metadata.name + "-" + id_str.substr(0, 8);

    // store spec snapshot; later LifecycleMgr queries availability
    // note: temporarily rename id to instance_id during serialization so downstream is consistent
...
std::optional<AbilityCR> ResourceManager::resolve_ability_cr_by_id(ID id) const {
    // 1) prefer instance_id from AbilityInstance table take spec_snapshot restore
    ...
    // 2) fallback: take from template table by CR id (compatible with old path / sub-ability id)
...
namespace {
// write CR validation failures to <home>/log/cr_validation.log for user troubleshooting
// independent of the glog main log; each record contains timestamp/filepath/CR name/reason
// The file is cleared at the start of each read_crs_into_db, always reflecting the current round's failure snapshot
...
std::variant<std::monostate, AbilityCR, DeviceCR> read_cr_from_yaml(
    ...
    if (is_ability(*kind)) {
        ...
        // force-check the spec.tasks field (avoid CR accidentally truncated, causing task field loss)
...
void ResourceManager::read_crs_into_db(const std::filesystem::path& path_crs) {
    ...
    // each round CR load clears cr_validation.log, make it reflect the current round's failure snapshot
    cr_validation_log_reset();

    // collect (ability_name, version, instance_name) triples successfully loaded this round,
    // after loading, use it to reconcile and clear orphan CR rows not in yaml.
    // design contract: CRs can only be injected via package/yaml; templates are not added/deleted via API at runtime.
    ...
    // recursive scan so package-embedded CRs can be placed in subdirs like crs/_packages/<pkg>/<version>/
    ...
    // Reconcile: delete all ability CR rows not in this round's yaml
    {
        constexpr char SQL[] =
        ...
        for (auto& id : stale_ids) {
            ...
            del.bind(1, id);
            del.exec();
            // also clean up associated labels/annotations/file info
            for (const char* tbl :
    ...
    // Reconcile: likewise clean up device CR orphan rows
    {
        constexpr char SQL[] =
...
void ResourceManager::update() {
    ...
    for (auto&& pkg : recursively_search_packages(packages_path)) {
        ...
        // idempotently mirror the package-embedded CRs and skills. The install path is already in add_package /
        // extract_package called in, but for framework start with an already-existing old package (did not go through
        // install path), supplement hereonce, let packages directory is source of truth.
...
// parent_id TEXT NOT NULL, -- dependent id
// parent_kind TEXT NOT NULL, -- "ability" or "device"
// parent_address TEXT NOT NULL, -- dependent address, default is localhost
// child_id TEXT NOT NULL, -- dependent id
// child_kind TEXT NOT NULL, -- dependent type, "ability" or "device"
// is_shared INTEGER NOT NULL, -- 0 indicates shared, 1 indicates exclusive
//   PRIMARY KEY (parent_id, parent_kind, child_id, child_kind)
...
expected<void, std::string> ResourceManager::occupy_ability(
    ...
    // resolve queries both AbilityInstance.spec_snapshot and AbilityCRBasic, supporting instance_id
...
expected<void, std::string> occupy_remote_device(
    ...
    // for a remote device, send the request to the corresponding framework
    nlohmann::json payload;
...
expected<msg_params::AbilityStorageInfo, std::string> ResourceManager::find_ability_storage_info(
    ...
    // prefer parsing from AbilityInstance.spec_snapshot (instance created, CR is a template copy)
    // fall back to template table (compatible with old path / sub-ability id)
...
bool ResourceManager::on_event_heartbeat(const Heartbeat& hb) {
    // syncupdate AbilityInstance table
    auto id_str = to_string(hb.id);
    auto state_str = std::string(to_string(hb.state));

    // instance exit semantics: enter Terminated i.e.destroyrow(regardless of singleton or replica)
    ...
    // only accept heartbeats from instances registered in the AbilityInstance table.
    //
    // historically there was a fallback: INSERT an "orphan heartbeat" when UPDATE hits 0 rows
    // placeholder row. That fallback would help previous-gen python zombie processes after framework restart
    // (reparented to init, PR_SET_PDEATHSIG expired and no longer fires) creating a ghost
    // instance — user sees one unrelated to any CR template, with spec_snapshot null
    // Running row — a debug nightmare.
    //
    // fix: directly drop the orphan heartbeat, return false so LifecycleMgr's HTTP layer returns
    // 410 Gone; the SDK self-destructs after receiving 410 consecutively (see ability-py-sdk heartbeat.py).
...
// autoStart: auto-spawn only once at framework start (auto_started=0)
...
// keepAlive: keep-alive (auto-restart after ability exits)
...
void ResourceManager::auto_start_ability() {
    ...
    // derive a new instance from a template and initiate lifecycle start
    // template_id: AbilityCRBasic.instance_id (template)
    // return the newly created instance_id; empty on failure
    auto spawn_instance_from_template = [&](ID template_id, const char* reason) -> std::optional<ID> {
        ...
        // singleton constraint: an active instance with the same ability_name already exists → skip
        ...
        // retry rate-limit (count by template id to avoid infinite retry on a single template crash)
    ...
    // autoStart: framework starts only once after startup
    ...
    // keepAlive: keep-alive; when the template has no active instance derived from it
...
void ResourceManager::on_register() {
    ...
    // at framework start, clean up non-Terminated legacy rows in the AbilityInstance table.
    // after a framework restart we cannot validate whether old instances are alive (PR_SET_PDEATHSIG + SubprocessMgr already
    // killed the actual orphan process), so all non-Terminated rows should be marked as
    // Terminated, avoid:
    // 1. singleton check get_heartbeats_by_ability_name mistakenly thinks there are running instances
    // 2. WebUI 'running instance' panel shows stale Running row
    {
        std::lock_guard _lk(m);
        auto now = std::chrono::duration_cast<std::chrono::seconds>(
        ...
        // at framework start, reset the auto_started flag: autoStart semantics are "each framework start
        // auto-spawns once", instead of "spawn only once for the entire CR lifecycle". Crash
        // protection is provided by in-memory start_records retry_count<3 within this session.
        try {
