...
namespace {
/// @brief the current implementation requires exact version match; no wildcard mechanism yet
/// @return whether the file is the corresponding version
...
public:
    ...
    // HTTP method: directly try to download; a 404 response means it does not exist
    // e.g. testability.company.com-0.1.0-x86_64.zip,
    // its detailed address is
    ...
    std::vector<char> download(const PackageSpec& spec) const override {
        ...
        // download succeeded, return data
